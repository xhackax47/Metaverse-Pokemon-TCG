...
local ultraCards = {"charizard", "blastoise","porygon", "omanyte", "omastar", "dragonite", "mewtwo", "mew", "snorlax", "articuno", "zapdos", "kabuto", "kabutops", "aerodactyl", "moltres", "dratini", "dragonair", "alakazam"}
...
QBCore.Functions.CreateUseableItem("boosterbox", function(source, item)
	...
	if Player.Functions.RemoveItem(item.name, 1, item.slot, item.info) then
        TriggerClientEvent("Cards:Client:OpenCards", source, item.name)
           ...
           Citizen.Wait(4000)
        TriggerClientEvent('QBCore:Notify', source, 'Tu as obtenu 4 packs de booster!')
            Citizen.Wait(1000)
    end
end)
...
QBCore.Functions.CreateUseableItem("boosterpack", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)  
        ...
        Citizen.Wait(4000)
        TriggerClientEvent('QBCore:Notify', source, 'Tu as obtenu 4 cartes!')
end)
...
AddEventHandler('Cards:Server:RemoveItem', function()
    ...
    if pack.amount == nil then
        TriggerClientEvent('QBCore:Notify', source, 'Tu n\'as pas de pack de booster!')
    else
...
AddEventHandler('Cards:Server:rewarditem', function()
   ...
   -- print(randomChance) -- if you wanna see the random chance
    ...
    if card ~= '' then        
        ...
        TriggerClientEvent('QBCore:Notify', source, 'Il y a un problème avec les cartes!')
    end 
end)
...
AddEventHandler('Cards:Server:GetPokemon', function(pokemon)
    ...
    if pokemon ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', source, "Tu as obtenu "..pokemonName.. "!!!")
            ...
            for k, v in pairs(Config.Badge[type].cards) do 
                if Player.Functions.GetItemByName(k) ~= nil then 
                    if Player.Functions.GetItemByName(k).amount < v then 
                       canBadge = false
                      TriggerClientEvent('QBCore:Notify', source, 'Reviens quand tu auras tous les objets pour le '..Config.Badge[type].label, 'error', 5000) 
                    end
                else 
                    canBadge = false
                    TriggerClientEvent('QBCore:Notify', source, 'Reviens quand tu auras tous les objets pour le '..Config.Badge[type].label, 'error', 5000)
                ...
                if canBadge then 
                    TriggerClientEvent('QBCore:Notify', source, 'Tu as obtenu un '..Config.Badge[type].label..'!', 'success', 10000)
                    for k, v in pairs(Config.Badge[type].cards) do
...
QBCore.Functions.CreateUseableItem("pokebox", function(source, item)
    ...
    TriggerEvent("qb-log:server:CreateLog", "pokebox", "PokeBox", "white", "Le joueur a ouvert la boîte **"..GetPlayerName(source).."** Citizen ID : **"..Player.PlayerData.citizenid.. "**", false)
end)
...
AddEventHandler("Cards:sellItem", function(itemName, amount, price)
    ...
    if xPlayer.Functions.RemoveItem(itemName, amount) then
        ...
        TriggerClientEvent("QBCore:Notify", source, "Tu as vendu " .. amount .. " " .. itemName .. " pour $" .. price, "success", 5000)
    end
end)
